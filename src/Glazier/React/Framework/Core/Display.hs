...
type Display m s r = s -> Z.ReactMlT m r

type FrameDisplay m s r = Display m (Z.Frame m s) r

-- | Gets the listeners for a particular 'GadgetId', usually for a
-- specific DOM element.
getListeners :: Z.GadgetId -> Z.Frame m s -> (DL.DList Z.Listener)
...
dirty :: Z.MonadReactor m => Z.Scene m v s -> m ()
dirty (Z.Obj ref its) =
    Z.doModifyIORef' ref (its.Z.plan.field @"currentFrameNum" %~ ((+ 1) . (`mod` JE.maxSafeInteger)))

rerender :: Z.MonadReactor m => Z.Scene m v s -> m ()
