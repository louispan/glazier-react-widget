...
andPrototype
    :: Prototype o1 ols d1 dtls p1 plns t1 trigs a1 acts c1 cmds
    -> Prototype o2 ols d2 dtls p2 plns t2 trigs a2 acts c2 cmds
    -> Prototype (Append o1 o2) ols
                 (Append d1 d2) dtls
                 (Append p1 p2) plns
                 (AppendUnique t1 t2) trigs
                 (AppendUnique a1 a2) acts
...
blank :: Prototype '[] ols '[] dtls '[] plns '[] trigs '[] acts '[] cmds
blank = Prototype (mempty, F.noop, F.ignore, F.idle)

statically :: F.Display dtls plns -> Prototype '[] ols '[] dtls '[] plns '[] trigs '[] acts '[] cmds
statically d = Prototype (d, F.noop, F.ignore, F.idle)

dynamically :: F.Gadgetry dtls plns a acts c cmds  -> Prototype '[] ols '[] dtls '[] plns '[] trigs a acts c cmds
dynamically g = Prototype (mempty, g, F.Trigger (Proxy, Proxy, mempty), F.idle)

triggering :: F.Trigger t trigs a acts -> Prototype '[] ols '[] dtls '[] pln t trigs a acts '[] cmds
triggering t = Prototype (mempty, F.Gadgetry (Proxy, Proxy, empty), t, F.idle)

creating :: F.Create o ols d dtls p plns acts -> Prototype o ols d dtls p plns '[] trigs '[] acts '[] cmds
